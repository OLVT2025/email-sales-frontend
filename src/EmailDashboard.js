...
//       try {
//         const response = await fetch(
//           `https://email-sales-backend.onrender.com/all-campaigns/`
//         );
//         const data = await response.json();
...
//             <AllCampaignsDetails campaigns={allCampaigns} />
//           )}
...
const EmailDashboard = () => {
  ...
  const CampaignDetailsView = ({ campaignDetails, isLoading }) => {
    if (isLoading) {
      return <div className="loading-indicator">Loading Campaign Details...</div>;
    }
    
    if (!campaignDetails) {
      return <div className="error-message">No campaign details available.</div>;
    }
    ...
    return (
      <div className="campaign-details-container">
        ...
        <Card className="campaign-details-card">
          <CardHeader>
            <CardTitle>Campaign Details: {campaignDetails?.campaign_id}</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="metrics-grid">
              ...
              <div className="metric-item">
                ...
                <small>{campaignDetails.summary.total_unsubscribes} unsubscribes</small>
              </div>
            </div>

            {campaignDetails.industry_data && campaignDetails.industry_data.length > 0 && (
            ...
            {campaignDetails.all_emails && campaignDetails.all_emails.length > 0 && (
